...
var station51 = new Gage(51, 2, sta51_spillZoneNames, sta51_spillZones, sta51_spillExtent, sta51_floodWatch, sta51_floodWarning, sta51Plot);
...
var station93 = new Gage(93, 4, sta93_spillZoneNames, sta93_spillZones, sta93_spillExtent, sta93_floodWatch, sta93_floodWarning, sta93Plot);
...
var station117 = new Gage(117, 1, sta117_spillZoneNames, sta117_spillZones, sta117_spillExtent, sta117_floodWatch, sta117_floodWarning, sta117Plot);
...
var station112 = new Gage(112, 6, sta112_spillZoneNames, sta112_spillZones, sta112_spillExtent, sta112_floodWatch, sta112_floodWarning, sta112Plot);
...
Plot.prototype.getSeries = function(time){
  ...
  //fcTimeIndex processing for animation
  for(var i=0;i<this.forecastData.length;i++){
...
function SpillZone(minSpill, maxSpill, numSpillLayers, spillRates, spillRatesToLayers, floodEvents, spillLayerNames, forecastData){
    ...
    this.retrievedLayers = new Array(numSpillLayers);
}
...
//////////GAGE OBJECT CODE////////////////////
function Gage(staNum, numSpillZones, spillZoneNames, spillZones, spillExtent, floodWatch, floodWarning, plot){
...
Gage.prototype.getSpillLayers = function(){
	if(this.spillLayersLoaded == false){
	    for(var i=0; i<this.numSpillZones; i++){
	        var spillZoneName = this.spillZoneNames[i];
	        var spillZone = this.spillZones[spillZoneName];
	        for(var j=0;j<spillZone.numSpillLayers;j++){
	            ...
	            spillZone.spillRatesToLayers[spillZone.spillRates[j]].styleMap = spill_vector_style_map;
	            map.addLayer(spillZone.spillRatesToLayers[spillZone.spillRates[j]]);
	        }
	    }
...
//download kml for forecasted spills
