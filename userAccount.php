			...
			<div class="panel panel-primary" style="top:80px;">
  				<div class="panel-heading">
  					...
  					<div class="panel-body">
						<form id="accountEditForm" action="userAccount.php?id=<?php echo $_SESSION['id']?>" method="post">
						...
						<div id="divPasswordChange">
							<div id="passwordChangeMessage" style="display:inline;visibility:hidden;"></div>
							<div id="pwChangeBtns" class="pull-right">
								<button class="btn btn-primary" id="btnPWChange">Change Password</button> 
							</div>
							<div id="formCell" style="position:relative;width:300px;left:25px;">
							</div>
						</div>
					</div>
				</div>
	...
	<script>
		$(document).ready(function(){
			...
			$('#btnUnsubscribe').on("click", function(){
				...
				if (unsubscribe == true) {
				    window.location.href = "unsubscribe.php?id=<?php echo $_SESSION['id'];?>";
				} 
			});
			$('#btnPWChange').on("click", function(){
				$('#btnEdit').attr("disabled", "disabled");
				$('#btnPWChange').css('visibility','hidden');
				$('#formCell').html('<form id="changePasswordForm" class="form form-horizontal" action="changePassword.php" method="post">' +
					...
					'<div class="form-group">' +
							...
							'&nbsp&nbsp<a class="btn btn-danger" id="btnCancel">Cancel</button>' +
					'</div>' +
					'</form>'
				);
				var script   = document.createElement("script");
				script.type  = "text/javascript";
				script.src   = "assets/js/pwChange.js";    // use this for linked script
				// script.text  = "alert('voila!');"               // use this for inline script
				document.body.appendChild(script);
			});

			$('#btnEdit').on("click", function(){
				//cannot change password while editing account info
			  	$('#btnPWChange').attr("disabled", "disabled");
